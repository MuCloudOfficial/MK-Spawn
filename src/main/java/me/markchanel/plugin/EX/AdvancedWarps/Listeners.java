...
public class Listeners implements Listener {
    ...
    private void WarpCommandListener(UserWarpEvent uwe) throws NullPointerException {
        ...
        IUser targetP = uwe.getUser();
        if (Config.Warps.containsKey(target)) {
            if(Config.RequiredPermissionWarps.containsKey(target)){
                if(!targetP.getBase().hasPermission(Config.RequiredPermissionWarps.get(target))){
                    targetP.sendMessage(Main.Prefix + ChatColor.RED + "你没有权限传送至目标地");
                    uwe.setCancelled(true);
                }
            } else if (Config.RequiredItemWarps.containsKey(target)) {
                List<Object> itemInfo = Config.RequiredItemWarps.get(target);
                boolean qualified = false;
                int amount = 0;
                for(ItemStack is : targetP.getBase().getInventory().getContents()){
            ...
            } else if (Config.RequiredMoneyWarps.containsKey(target)) {
                if(targetP.getMoney().doubleValue() < Config.RequiredMoneyWarps.get(target)){
    ...
    private void WarpModifyListener(WarpModifyEvent wme) throws NullPointerException {
        ...
        switch (wme.getCause()){
            case CREATE:
                ...
                File targetCreateFile = new File(Config.EssC.getConfigFile().getParentFile().getAbsolutePath() + File.separator + "warps" + File.separator + wme.getWarpName() + ".yml");
                try {
                    ...
                    f.set("name",wme.getWarpName());
                    f.set("lastowner",wme.getUser().getBase().getUniqueId());
                    f.save(targetCreateFile);
                } catch (IOException | InvalidConfigurationException e) {
                    e.printStackTrace();
                }
                target.sendMessage(Main.Prefix + ChatColor.GREEN + "你创建了一个新地标 " + wme.getWarpName() + " 位置: " + wme.getNewLocation());
                break;
            case DELETE:
                Config.Warps.remove(wme.getWarpName());
                Config.getWarpFile(wme.getWarpName()).delete();
                target.sendMessage(Main.Prefix + ChatColor.YELLOW + "你删除了一个地标 " + wme.getWarpName());
                break;
            case UPDATE:
                ...
                target.sendMessage(Main.Prefix + ChatColor.YELLOW + "你更新了" + wme.getWarpName() + "地标的位置" + " 新位置: " + wme.getNewLocation());
                break;
        }
    }
